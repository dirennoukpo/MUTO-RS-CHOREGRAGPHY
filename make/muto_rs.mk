...
muto-rs-deploy: ## Deploy the MUTO-RS stack [SSH_HOST=user@host]
	...
	@ssh $(SSH_HOST) 'mkdir -p ~/muto_rs/docker ~/muto_rs/config'
	@scp -q docker/docker-compose.muto_rs.yml $(SSH_HOST):~/muto_rs/docker/
	@if [ -f config/.env.muto_rs ]; then \
		scp -q config/.env.muto_rs $(SSH_HOST):~/muto_rs/config/.env.muto_rs; \
	else \
		scp -q config/.env.muto_rs.example $(SSH_HOST):~/muto_rs/config/.env.muto_rs; \
	fi
