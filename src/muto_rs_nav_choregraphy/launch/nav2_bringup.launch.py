...
def _find_repo_root() -> Path:
    ...
    if env_repo:
        ...
        if (candidate / "src" / "muto_rs_nav_choregraphy").exists():
            return candidate

    current_file = Path(__file__).resolve()
    for parent in current_file.parents:
        if (parent / "src" / "muto_rs_nav_choregraphy").exists():
            return parent

    workspace_repo = Path("/workspace")
    if (workspace_repo / "src" / "muto_rs_nav_choregraphy").exists():
...
def generate_launch_description():
    ...
    default_map = repo_root / "src" / "muto_rs_nav_choregraphy" / "maps" / "electronics_room.yaml"
    default_params = repo_root / "src" / "muto_rs_nav_choregraphy" / "params" / "nav2_params.yaml"
